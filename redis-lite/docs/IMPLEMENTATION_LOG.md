...
## 坑 11：ZRANK 攻克记 —— 从「连续失败 11 轮」到「第一次穷举就定位」

这是本项目最长的一段调试，也是最有价值的一段。

### 背景与第一次取舍

`ZRANK` 要做到 O(log N) 必须用带 span 的跳表。第一版实现反复出错，
表现是 **「rank 时对时错」** —— 换个层数分布就换一种错法。
连续调试 **11 轮未收敛**后，我做了诚实取舍：把跳表移出构建，
用 `std::set` 保证交付，并把缺口、影响面、补齐路径写进技术报告。
...
然后枚举「N 个元素各自层数」的**全部组合**（N=4/6/7 配 maxLevel=3、
N=5 配 maxLevel=5，共 **6122 组**），每组逐步插入并校验全部不变式。
...
对比之前：人工构造用例 + 手工推演，11 轮都没定位到。

### 定位到的三个问题（按发现顺序）

**① 空表的 header span 未初始化（真实 bug）**

```
Validate failed: level 0: zero span on node m0
```

按定义 `span[i] = rank(target) - rank(self)`；空表时 target 是虚拟表尾
（rank = 1），故 `header.span[i]` 应为 **1**，而我初始化为 0。
插入第一个元素时读它算「原后继排名」，得到 0 而非 1，于是算出零 span。

**② `Validate` 检查了错误的字段（我的自检又错了）**

```
level 1: span mismatch on m0 (span=2, expected 1)
```

按约定 `span[i]` 存在**发出指针的那个节点自己身上**：
`span[i](p) == rank(p->forward[i]) - rank(p)`。
而校验读的是 `target->span[i]` —— **算法是对的，是校验读错了字段**。
...
同一错误在 **3 处复制**：插入搜索、`ZRANK`、`ZRANGE` 定位。

**为什么它能躲过 11 轮调试**：第 0 层每个节点的 span 都是 1，
所以 `x->span[0] == x->forward[0]->span[0]` —— **纯单层场景永远正确**。
手写的用例恰好都落在单层路径上，只有跨层时才偏。
这就是「rank 时对时错」的完美解释，也是**穷举测试不可替代**的最好例证。

### 结果

- 跳表单独测试：**17 用例 / 2851 断言**全过（含 6122 组穷举配置）
- ZSet 改为「字典 + 跳表」双索引后：全项目 **114 用例 / 6199 断言**全过
- `ZRANK` 实测：规模 ×1000，耗时仅 ×16.4（亚线性，符合 O(log N)）

### 可迁移的三条结论

1. **先定义正确性，再调算法。** 不变式写成 `Validate()` 后，问题自己暴露。
2. **穷举代替人工挑用例。** 对「层数分布敏感」的 bug，人工覆盖度远远不够。
3. **字段语义要能一句话说清。** `span[i]` 挂在谁身上、代表什么，
   必须唯一无歧义 —— 歧义会以最难查的方式显形。

---

## 坑 12：判重放在主插入路径上，让 ZADD 退化成 O(N)

跳表按 `(score, member)` 排序，只有 member 无法直接定位，
所以「member 是否已存在」只能线性扫 —— O(N)。

我把判重留在了 `SkipList::Insert` 里，而 ZSet 又调用它，
于是**每次 ZADD 都多一次 O(N) 扫描**，尽管 ZSet 的字典已经 O(1) 判过重。
N=100 万时这条路径完全不可用（建表退化成 O(N²)）。

**修复**：拆成两个入口——

| 入口 | 行为 | 使用者 |
|---|---|---|
| `InsertNew()` | 假定 member 不存在，**不判重**，O(log N) | ZSet（字典已判重） |
| `Insert()` | 含 O(N) 判重，已存在则更新 | 跳表独立使用 / 单元测试 |

**教训**：一个结构被**两层**使用时，要明确「谁负责哪项校验」。
职责重叠不会带来安全，只带来额外开销；职责遗漏才带来 bug。两者都要写清楚。

---

## 坑 13：用 PowerShell 字符串替换修改含中文的源码 → 文件损坏

用 `Get-Content -Raw` + `-replace` + `Set-Content -Encoding UTF8`
替换源码里一处函数名后：

- 中文注释变乱码（`zset.cpp 鈥?鏈夊簭闆嗗悎瀹炵幇`）
- **换行符被吞掉**，注释与后面的语句并成一行
- 后果：`CMakeLists.txt` 里 3 处 `set()` / `file()` / `add_executable()`
  被并进注释行 —— **源码收集为空、服务端目标消失**，构建直接失败

**教训**：**不要用 shell 字符串操作修改含非 ASCII 内容的文件。**
为此损失了数轮往返（改坏 → 编译报一堆莫名的
`'index_' was not declared` → 才发现文件结构已损坏）。

正确做法：用能正确处理 UTF-8 的工具做精确替换，
改完后**扫一遍编码损坏特征**（如 grep `锛|鈥|鏈|鏂`）。
...
| **11（ZRANK 攻克记）** | **诚实取舍 + 用穷举测试代替人工推演；跨 14 轮调试的完整闭环** |
| 12（判重污染热路径） | 分层设计里「职责边界」的敏感度 |
| 13（工具改坏文件） | 工具链陷阱意识：改完要验证，不能假设 |
...
## 坑 14：SkipList 析构漏掉头节点（ASan 在 CI 里抓到）

```
Indirect leak of 128 byte(s) in 1 object(s) allocated from:
    #1 rl::SkipNodeCreate(...)   src/skip_list.cpp
    #2 rl::SkipList::SkipList()  src/skip_list.cpp
SUMMARY: AddressSanitizer: 2756096 byte(s) leaked in 18456 allocation(s)
```

**根因**：`~SkipList()` 只调了 `Clear()`。而 `Clear()` 释放链表上的数据节点
并**复位**头节点 —— 头节点本身是构造时 `new` 出来的，不受 Clear 管理。
每个实例泄漏约 128 字节（头节点的 32 个 forward + 32 个 span + 节点体）。

**为什么单测发现不了**：内存泄漏不改变任何可观测行为。
单元测试全绿（114 用例 / 6199 断言），只有 sanitizer 能看见。

**教训**：**「谁分配、谁释放」要在类内闭环。**
`Clear()` 的语义是「清空数据」，析构的语义是「归还全部资源」——
两者不是一回事，把析构实现成 Clear 是常见的偷懒式错误。

---

## 坑 15：验证脚本没有真正断言 —— 第三次同类问题

**现象**：`verify_all.sh` 报 `ALL PASS`，而 ASan 单测其实**一直在失败**
（就是坑 14 的泄漏）。

**根因**：脚本里写成

```bash
./build-debug/bin/rl-tests > log 2>&1 && tail -3 log     # 失败时 tail 被短路
bash scripts/test_shutdown.sh ... | tail -8
[ $? -ne 0 ] && FAILS=$((FAILS+1))                        # $? 是 tail 的退出码
```

两个问题叠加：**(a)** 管道后的 `$?` 取的是最后一个命令（`tail`）的状态，
不是被测程序的；**(b)** `&&` 让失败时的日志也不打印，排查时看不到线索。

**修复**：抽出 `run_step()`，每步单独取退出码，失败时打印关键行。

**这是本项目第三次同类问题**：

| 次数 | 场景 | 表现 |
|---|---|---|
| 1 | `test_shutdown.sh` | 二进制不存在时服务器根本没启动，脚本报 ALL PASS |
| 2 | `e2e_zset.sh` | 连上了上一轮遗留的旧服务端进程，得出与代码不符的结论 |
| 3 | `verify_all.sh` | 退出码取错 + 日志被短路，泄漏测试失败被吞掉 |

**共同教训**：

> **验证步骤必须在「被测对象没有真正运行 / 失败」时大声失败。**
> 一个不会失败的检查，比没有检查更危险 —— 它制造虚假信心，
> 而且会在最关键的时候（交付前、面试演示时）才暴露。

写法上的三条具体要求：
1. 每个被测命令**单独取 `$?`**，不要跨管道、不要用 `&&` 串联后取值
2. 断言「被测进程真的起来了」而不只是「端口可达」
3. 失败时打印日志片段，否则排查成本高于重跑