...
| 数据结构 | 字符串、整数编码、**有序集合（ZSet = 字典 + 带 span 的跳表）** |
...
| 操作 | 本实现 | Redis |
|---|---|---|
| `GET` / `SET` / `DEL` | 期望 O(1) | O(1) |
| `ZADD` | 期望 O(log N) | 期望 O(log N) |
| `ZSCORE` | O(1)（字典） | O(1) |
| `ZRANGE` / `ZRANGEBYSCORE` | O(log N + M) | O(log N + M) |
| **`ZRANK`** | **O(log N)**（带 span 的跳表） | O(log N) |
| `KEYS` | O(N) | O(N) |

`ZRANK` 的 O(log N) 有实测证据（`tools/bench_zrank.cc`）：
规模从 1 千扩到 100 万（**×1000**），单次耗时仅从 0.26µs 增到 4.23µs（**×16.4**）——
亚线性增长。若真是 O(N)，100 万规模应约 257µs。

> 这个能力是**攻克回来的**：第一版跳表的 span 更新连续调试 11 轮未收敛，
> 当时选择诚实取舍（记录缺口、移出构建、保证交付）；后来改用
> **穷举层数组合的测试**（6122 组），第一次运行就定位到根因 ——
> 累加排名时读错了字段，且该错误在 3 处复制，被「第 0 层 span 恒为 1」的巧合掩盖。
> 完整过程见 `docs/IMPLEMENTATION_LOG.md` 坑 11。
